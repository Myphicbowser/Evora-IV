...
*/
	if(buckled_mob && padding_material)
		...
		if(isnull(stool_cache[cache_key]))
			...
			stool_cache[cache_key] = I
		overlays |= stool_cache[cache_key]
	if(buckled_mob && padding_material && dir == NORTH)
		var/padding_cache_key = "[base_icon]-padding-[padding_material.name]-over"
		if(isnull(stool_cache[padding_cache_key]))
			var/image/I = image(icon, "[base_icon]_padding_over")
			if(material_alteration & MATERIAL_ALTERATION_COLOR)
	...
	if(dir == NORTH && buckled_mob && !padding_material)
...
/obj/structure/bed/chair/RightClick(mob/user)
...
/obj/structure/bed/chair/comfy/captain
	...
	buckle_movable = 1
/*
/obj/structure/bed/chair/comfy/captain/throne
	var/datum/announcement/priority/crew_announcement = new

/obj/structure/bed/chair/comfy/captain/throne/verb/announce
	set name = "Announce"
	set category = "Throne"
